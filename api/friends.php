...
$get_friends_id = $mysqli->prepare("SELECT U.name, U.email, U.id FROM users U INNER JOIN friends F"
                                   ." ON U.id = F.friend1_id WHERE F.friend_blocked = 0 and F.friend2_id=?"
                                   ." UNION SELECT U.name, U.email, U.id FROM users U INNER JOIN friends F"
